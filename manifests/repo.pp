...
define yum::repo (
  ...
  if $baseurl == 'UNSET' {
    $my_baseurl = "${repo_server_protocol}://${repo_server}/${repo_server_basedir}/${name}/${::lsbmajdistrelease}/${::lsbminordistrelease}/${::env}/\$basearch"
  } else {
    $my_baseurl = $baseurl
  }
  ...
  # http://yum.domain.tld/keys/RPM-GPG-KEY-CUSTOMREPO-5-8
  if $gpgkey == 'UNSET' {
    if $use_gpgkey_uri == 'true' {
      $my_gpgkey = "${gpgkey_url_proto}://${gpgkey_url_server}/${gpgkey_url_path}/${gpgkey_file_prefix}-${upcase_name}-${::lsbmajdistrelease}-${::lsbminordistrelease}"
    } else {
      $my_gpgkey = $gpgkey
    }
  }
  ...
  if ($gpgcheck == '1') and ($use_gpgkey_uri == 'true') {
    ...
    yum::rpm_gpg_key { $upcase_name:
      ...
      gpgkey     => "${my_gpgkey_local_path}/${gpgkey_file_prefix}-${upcase_name}-${::lsbmajdistrelease}-${::lsbminordistrelease}",
